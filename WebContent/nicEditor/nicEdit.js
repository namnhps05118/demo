...
var nicUploadButton=nicEditorAdvancedButton.extend({nicURI:"nicUpload.php",addPane:function()
{
...
setTimeout(this.addForm.closure(this),50)},addForm:function()
{
...
A.write('<div style="font-size: 14px; font-weight: bold; padding-top: 5px;">Chọn ảnh</div>');
...
this.myStatus=new bkElement("div",this.myDoc).setStyle({textAlign:"center",fontSize:"14px"}).appendTo(this.myBody)},startUpload:function(){this.myForm.setStyle({display:"none"});
this.myStatus.setContent('<img src="ajax-loader.gif" style="float: right; margin-right: 40px;" /><strong>Đang uploading...</strong><br />Vui lòng đợi trong giây lát !');
this.myForm.submit();

setTimeout(this.makeRequest.closure(this),this.requestInterval)},makeRequest:function()
{
...
},update:function(C)
{
    ...
    else
    {
        if(C.url)
        {
            ...
            if(this.im)
            {
                this.im.setAttributes({src:C.url,width:(A&&C.width)?Math.min(A,C.width):""})}this.removePane()
            }
            else
            {
                if(C.error){this.requestInterval=false;
                this.setProgress(100);
                alert("Lỗi upload hình ("+C.error+").");
                this.removePane()
            }else
            {
            if(C.noprogress)
